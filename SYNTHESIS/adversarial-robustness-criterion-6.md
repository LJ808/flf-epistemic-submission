...
A methodology that only defends against (1) leaves half the criterion untested. Most of this document's prior draft covered (1) thoroughly. This revision adds (2).
...
7. Underpowering — can't fix wide CIs, but puts the numbers in the sentence so wide intervals are visible
...
Partially, and more successfully than Test 1. Peter's original claim — "it seems epidemiologically impossible" — is exactly the kind of confidence-language a debater chooses *because* it sounds authoritative. The E-Prime rewrite ("Assuming constant 3.5-day doubling, a November 11 start implies 256× the case count actually observed") **cannot be delivered with the same rhetorical confidence**, because it now requires stating an assumption explicitly. A debater optimizing for persuasive weight over evidential weight loses that option once the assumption must surface in the same breath as the conclusion.

**Verdict:** This is the strongest case in the vault for source-side gaming resistance. The rhetorical engineering specifically depended on hiding the assumption inside confident phrasing; E-Prime structurally blocks that specific move. This matches covid-001's own analysis section, which independently identified the detection-rate assumption as the thing Peter's framing buried.

### Test 3: Institutional Hedge-Then-Reassure Pattern (Black Holes Case)

**Adversarial scenario:** An institution (CERN) facing public anxiety has incentive to present a reassuring conclusion even where the underlying argument has real gaps, because the institutional cost of public panic exceeds the cost of an imperfect safety argument. The FAQ format itself — questions answered briefly, confidently — is optimized for public reassurance, not epistemic completeness.

**Does E-Prime catch this?**

This is the most instructive test in the vault, because it shows E-Prime's defense working exactly as designed and exactly where its limits are. The rewrite of blackhole-002 ("the same extra-dimension theoretical framework predicting microscopic black hole formation also predicts Hawking-radiation evaporation on a sub-accretion timescale") exposes that the FAQ's two-part reassurance (formation is improbable; even if it happens, it's safe) **shares one theoretical root**, which the original FAQ structure obscures by presenting them as two separate reassurances. A reader trusting "two independent safety arguments" gets corrected by the rewrite into seeing "one argument, stated twice."

**What this doesn't catch:** Whether CERN's institutional incentive to reassure shaped which arguments made it into the public FAQ versus the full LSAG 2008 report. The vault's own missing-perspectives report already flags that the LSAG report itself was never ingested — meaning **the strongest test of institutional source-gaming (comparing the public-facing reassurance to the full technical safety case) remains undone**, not because E-Prime failed, but because this vault build never ingested the comparison source.

**Verdict:** E-Prime correctly defeats source-gaming *within* a single document (catching the false-independence framing). It cannot defend against source-gaming *across* documents the methodology never ingested (catching whether the FAQ selectively omits material from the fuller report). This is a real, named limitation — not a failure of the technique, but a scope boundary that needs disclosure to judges rather than silence.

---

## PART C: What This Means for the Criterion, Stated Without Inflation

FLF's Criterion 6 has two clauses. This vault's evidence supports different confidence levels for each:
...
**"The methodology resists being gamed by sources optimizing to mislead"** — **Partially supported, with a named boundary.** Test 2 (COVID rhetorical engineering) shows clear, demonstrated resistance. Test 3 (institutional FAQ) shows resistance *within* a document but an unaddressed gap *across* documents. Test 1 (cohort gaming) shows the methodology converts invisible gaming into a flagged-but-unresolved question, which is real value but falls short of "resistance" in the strong sense.

**Honest summary for judges:** E-Prime is a detection-and-disclosure mechanism, not a fraud-proof filter. It cannot recover information a source never disclosed. What it reliably does: make the *place* where motivated design would have to hide structurally visible, so that the absence of disclosure becomes itself a flagged data point rather than smooth prose. This is the same finding Part A reached for reader-side gaming, now confirmed to hold — with one explicit cross-document blind spot — for source-side gaming as well.
...
3. Run the E-Prime rewrite. Check: does the omitted information now have a structurally visible gap where it should appear (Test 1 pattern), does the confident framing become impossible to sustain without stating the buried assumption (Test 2 pattern), or does the gaming require comparison to a document never ingested (Test 3 pattern)?
4. Log which pattern applies. Pattern 2 is the strongest result; Pattern 1 is real but partial; Pattern 3 means the vault's scope, not the method, is the limit — and that should be stated as a scope gap to extend, not a methodology failure to hide.
